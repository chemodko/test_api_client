...
from src.utils.constants.messages import StatusMessage, GameMessage
...
def dlc_factory(num: int = 2):
    return [DLCFactory() for _ in range(num)]
...
class GameFactory(BaseModel):
    ...
    price: float = Field(default_factory=random_float)
    publish_date: str = Field(default=datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S'))
    rating: int = Field(default_factory=random_number)
    required_age: int = Field(alias="requiredAge", default_factory=random_number_1_18)
