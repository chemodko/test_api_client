...
@allure.feature("Getting game info")
class TestGetGame:
    @allure.title("Getting game info")
    ...
    @allure.title("Getting game info without authorization")
    def test_get_game_info_unauthorized(self, game_controller_auth_post_game_delete):
        game_id = game_controller_auth_post_game_delete.get_games_list()[0].game_id
        game_controller_auth_post_game_delete.logout()
        game_controller_auth_post_game_delete.get_game_info(game_id=game_id, status_code=401, model=UnauthorizedError)

    @allure.title("Getting game info that not exist")
